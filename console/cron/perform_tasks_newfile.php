...
echo "Closing all Chrome Drivers if working backround...\n";
`taskkill /im chromedriver.exe /f`;

echo "Closing all Chrome if working backround...\n";
...
while (true)
{
	...
	$taskQuery = 'SELECT * FROM `tasks` WHERE `due`<='.time().' ORDER BY `due` LIMIT 1';
	//$taskQuery ='SELECT * FROM `tasks` WHERE `id`='.'221138'.' ORDER BY `due` LIMIT 1';
	echo $taskQuery;

	//sleep(120);

	$tasks	= $dbo->execute($taskQuery);
	...
	if ($task['type'] == 'create_bot') {
		...
		$webdriver->get('chrome://extensions/?options=enhldmjbphoeibbpdhmjkchohnidgnah');
		sleep(3);

		$extensionoptions	= $webdriver->findElementBy(LocatorStrategy::cssSelector, 'extensionoptions');
		...
		if ($settings['mail_source'] =='auto' || $_mail_auto) {
			$settings['mail_source'] = $config['mail_sources'][rand(0, 0)];
			$_mail_auto = true;
		}
		...
		if ($settings['mail_source'] == 'temp-mail.org') {
			...
			echo 'Creating temp-mail.org account...'."\n";
			...
			echo 'Created temp-email.org account: ('.$bot['email'].")\n";
		...
		//-------------------------------------------------------------------------------------------------------------
			...
			echo 'Creating Mytemp.email account...'."\n";
			...
			echo 'Created Mytemp.email account: ('.$bot['email'].")\n";
		...
		if ($settings['mail_source'] == 'temp-mail.org') {
			...
			echo 'Waiting for activation mail...'."\n";
			...
			echo 'Activation mail found. Reading...'."\n";
			...
			echo 'Activation link is: '.$activation_link."\n";
		...
		//---------------------------------------------------------------------------------------------
		echo 'Going to the activation link...'."\n";
	...
	//-------------------------------------------------------------------------------------------------------------
		...
		//---------------------------------------------------------------------------------------------
		sleep(1);
		if (sizeof($webdriver->findElementBy(LocatorStrategy::cssSelector, '.tv-dialog a[href="/house-rules/"]'))) {
	...
	//-------------------------------------------------------------------------------------------------------------
		...
		$bot	= $dbo->execute('SELECT * FROM bots WHERE status=\'active\' AND id NOT IN(SELECT bot_id FROM article_likes WHERE article_id='.$article['id'].') ORDER BY rand() LIMIT 1');
		$bot	= $bot[0];
	...
	//-------------------------------------------------------------------------------------------------------------
		...
		echo "Visitling article...\n";
		echo "	Link: ".$article['link']."\n";
		echo "	Proxy: ".$proxy['domain']."\n";
	...
	//-------------------------------------------------------------------------------------------------------------
        ...
        echo "before web drovere in reply comment";
        sleep(3);      
		//---------------------------------------------------------------------------------------------
		...
		echo 'Cevap verilecek bot yorumu araniyor: '.$task['extra']['target_bot_username'].'...'."\n";
		...
		while (true) {
			...
			if (sizeof($webdriver->findElementsBy(LocatorStrategy::cssSelector, 'a[data-username="'.$task['extra']['target_bot_username'].'"]'))) {
				...
				echo 'Hedef bot yorumu bulundu: '.$task['extra']['target_bot_username']."\n";
				...
				echo "Bot, diger botun yorumuna cevap verdi.\n";
			...
			if ($comment_blocks_count == sizeof($comment_blocks)) {
				echo 'Hedef bot yorumu bulunamadi'."\n";
				break;
			}
